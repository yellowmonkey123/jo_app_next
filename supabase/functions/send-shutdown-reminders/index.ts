...
interface DailyLogData {
  shutdown_reminder_sent_at: string | null;
}
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const timeEligibleUsers = potentialUsers.filter((user: UserProfile) => {
      ...
      return localHour === TARGET_SHUTDOWN_HOUR;
    });
    ...
    for (const user of timeEligibleUsers) {
      ...
      try {
        const { data: logData, error: logError } = await supabaseAdmin
          .from('daily_logs')
          .select('shutdown_reminder_sent_at')
          .eq('user_id', userId)
        ...
        if (logData && logData.shutdown_reminder_sent_at) {
      ...
      console.log(`[User: ${userId}] Attempting to send shutdown SMS to ${user.phone}...`);
      ...
      try {
        ...
        if (!response.ok) {
          ...
          console.log(`[User: ${userId}] Shutdown SMS sent successfully. SID: ${responseData.sid}`);
            ...
            .upsert(
              {
                ...
                shutdown_reminder_sent_at: nowIsoString
              },
              {
